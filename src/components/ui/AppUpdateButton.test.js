...
describe("AppUpdateButton", () => {
  ...
  test("checks first, then installs the newer APK in-app on the next click", async () => {
    ...
    await waitFor(() =>
      expect(checkForAppUpdate).toHaveBeenCalledWith({
  ...
  test("uses a distinct app-update affordance", () => {
    ...
    expect(
      container.querySelector(".lucide-circle-arrow-up"),
    ).toBeInTheDocument();
    expect(
  ...
  test("explains when the public update source is unavailable", async () => {
    ...
    expect(
      screen.getByRole("button", { name: "retryUpdateCheck" }),
    ).toBeInTheDocument();
  });
  ...
  test("lets the user dismiss an actionable update notice without losing the update action", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "appUpdateAction" }));
    expect(
      await screen.findByText("updateAvailableMessage v1.1.0"),
    ).toBeInTheDocument();

    fireEvent.click(
      screen.getByRole("button", { name: "dismissUpdateMessage" }),
    );

    expect(
      screen.queryByText("updateAvailableMessage v1.1.0"),
    ).not.toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "updateToVersion v1.1.0" }),
    ).toBeInTheDocument();
  ...
  test("keeps an actionable update notice visible until it is dismissed", async () => {
    ...
    expect(
      screen.getByText("updateAvailableMessage v1.1.0"),
    ).toBeInTheDocument();
  });
